    ...
    try:
        ...
        for i in currency_dict.keys():
            if i in voice_input:
                ...
                print(f"В данну годину курс {i} відображается в продажі за {buy}, а в покупці за {sell}")
                break
    except:
        for i in currency_dict.keys():
            if i in voice_input:
...
def artem():
    try:
        for i in currency_dict.keys():
            try:
                if i in voice_input:
                    b = data[currency_dict[i]['rateBuy']]
                    s = data[currency_dict[i]['rateSell']]
                    print(f'Валюта {i} Покупка {b} Продаж {s}')
                    print('done')
            except:
                print('err 1')
    except:
    ...
    for i in currency_dict.keys():
        if i in voice_input:
            try:
                buy = data[currency_dict[i]]['rateBuy']
                sell = data[currency_dict[i]]['rateSell']
                print(f'Курс {i} при купівлі: {buy}, при продажі:{sell}')
            except:
                print('Помилака. Спробуйте пізніше')
    ...
    for i in currency_dict.keys():
        if i in voice_input:
            try:
                buy = data[currency_dict[i]]['rateBuy']
                sell = data[currency_dict[i]]['rateSell']
                print(f'На сегодняшний день стоимоть {i} продажи составляет {sell}, а покупки {buy}')
            except:
                print('Слишком много запросов, повторите попытку позже')
    ...
    for i in currency_dict.keys():
        try:
            if i in voice_input:
                buy = data[currency_dict[i]['rateBuy']]
                sell = data[currency_dict[i]['rateSell]']]
                print(f'At this moment: {i}, Buy:{buy}, Sell:{sell}')
        except:
            print('Too many requests, please try again later, or see an error in the console')
...
def danya():
    try:
        for i in currency_dict.keys():
            if i in voice_input:
                r = data[currency_dict[i]]['rateBuy']
                
                r2 = data[currency_dict[i]]['rateSell']
                print(f'курс {i} сейчас в покупке за {r} а в продаже за {r2}')
            # if i not in currency_dict:
            #     tts.engine.say("не найдено")
    except:
        print('err')
...
def denisk():
    for i in currency_dict.keys():
        if i in voice_input:
            data[currency_dict[i]]['rateByy']
            data[currency_dict[i]]['rateSell']
            print(f')
...
def vanya():
    for i in currency_dict.keys():
        if i in voice_input:
            try:
                buy = data[currency_dict[i]]['rateBuy']
                sell = data[currency_dict[i]]['rateSel']
                print(f'Валюта: {i}, покупка: {buy}, продажа: {sell}')
            except:
                print('Ошибка')
